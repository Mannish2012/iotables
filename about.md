...
however, is a matrix. In fact, this matrix, called the **consumption matrix** is the product $C\\mathbf{x}$,
where $C$ is the matrix of the coefficients of the vectors, and **x**, is the production vector.

The equation is, hence, $\\mathbf{x} = C\\mathtbf{x} + d$, where $$\\mathbf{d}$$ is the *final demand vector*. Now if the matrix
