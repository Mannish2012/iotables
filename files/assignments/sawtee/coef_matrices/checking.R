...
forward_linkage <- function(year, type){
  ...
  # Creating the technical coefficients matrix
  # create a temporary named list to store variable values

  # for backward linkage, we need gross output of sector 'j' in the denominator
  ## that is obtained by the following code
  temp_var_list <- lapply(names(dat), function(sector){
    # intermediate sales from sector i to sector j/gross output of sector i
    dat[sector]/sum(dat[sector])
  }) |> setNames(industry_names_abb)
  
  # for forward linkage, we need gross output of sector 'i' in the denominator
  # that is obtained by 
  gross.sector_i <- rowSums(dat)
  # the output coefficients matrix is thus: 
  output_coef_matrix <- dat/gross.sector_i
  
  # DIRECT impact b/c of unit increase in gross output of sector 'i' (FL)
  
  

  # checking the numerator and denom values of temp_var_list
  check1 <- lapply(names(dat), function(s) dat[s])
  check2 <- lapply(names(dat), function(s) sum(dat[s]))
  ...
  input_coeff_matrix <- as.matrix(eval(str2expression(temp_var_index)))*0.99
  ...
  switch(type,
         original = dat,
         chk1 = check1[[1]], # dat[sector]
         chk2 = check2, # sum(dat[sector]), gross output of sector 'j'
         chk3 = rowSums(dat), # gross output of sector 'i'
         chk4 = colSums(dat), # same as chk2
